...
function newUser(userid) {
	if (userid != client_id) {
		$('#userlist').append(
				"<option value='" + userid + "' id='user_" + userid + "'>"
						+ userlist[userid].Name + "</option>");
	}
	$('#left-userlist').append(
			"<li id='inroom_" + userid + "'><a href='javascript:selectUser("
					+ userid + ")'>" + "<img src='" + userlist[userid].Avatar
...
function newMsg(fromId, content, color) {
	...
	if (fromId == 0) {
		...
		var html = '';
		// $("#msg-template .userpic").html("<img src='" +
		// userlist[fromId].Avatar + "' width='50' height='50'>")
		if (client_id == fromId) {
			html += '<span style="color: orange">我说: </span>: ';

		} else {
			html += '<span style="color: orange"><a href="javascript:selectUser('
					+ fromId + ')">' + userlist[fromId].Name;
			html += '</a></span> '
		}
		html += content + '</span>';
